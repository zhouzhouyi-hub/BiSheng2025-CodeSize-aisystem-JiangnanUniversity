...
void ReorderData::setSectionOrder(BinaryContext &BC,
  ...
  uint64_t Offset = 0;
  uint64_t Count = 0;
  std::unordered_map<std::string, unsigned> symbolAccessCount;
  ...
  for (; Begin != End; ++Begin) {
    ...
    std::string symbolName = BD->getName().str();

    // 增加访问次数
    symbolAccessCount[symbolName]++;
  ...
  outs() << "BOLT-INFO: reorder-data: " << Count << "/" << TotalCount
         ...
         << Offset << " hot bytes\n";
  outs() << "BOLT-INFO: Symbol Access Counts:\n";
  for (const auto &entry : symbolAccessCount) {
    outs() << "  " << entry.first << " accessed " << entry.second << " times\n";
  }
}
...
void ReorderData::runOnFunctions(BinaryContext &BC) {
  static const char *DefaultSections[] = {".rodata", ".data", ".bss", ".sdata"};
