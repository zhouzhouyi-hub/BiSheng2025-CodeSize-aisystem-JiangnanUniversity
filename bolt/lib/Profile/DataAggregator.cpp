...
std::error_code DataAggregator::parseMMapEvents() {
  ...
  for (MMapInfo &MMapInfo : llvm::make_second_range(make_range(Range))) {
    if (BC->HasFixedLoadAddress && MMapInfo.MMapAddress) {
      ...
      BinaryMMapInfo.insert(std::make_pair(MMapInfo.PID, MMapInfo)); // YANGZI20250403 直接假如mmap避免PERF2BOLT-ERROR: could not find a profile matching binary
...
//    BinaryMMapInfo.insert(std::make_pair(MMapInfo.PID, MMapInfo)); YANGZI20250403
  }
