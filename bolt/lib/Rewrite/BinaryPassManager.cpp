...
#include "bolt/Passes/RISCV/RISCVGVN.h"
#include "bolt/Passes/RISCV/RISCVSimplifyCFG.h"
...
goon:
  ...
  errs() << "SizeChanged = " << llvm::utohexstr(BC.sizeChange, /*LowerCase=*/true) << "\n";

  outs() << "--------------------------------------\n";
  errs() << formatv("Total shrink: {0} bytes\n", totalShrink);
  errs() << formatv("Total grow:   {0} bytes\n", totalGrow);
}
