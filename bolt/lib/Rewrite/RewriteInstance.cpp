...
extern cl::OptionCategory InferenceCategory;//dhf-modify
...
static cl::opt<bool> MaxCompress(  // YANGZI20250405
    "max-compress",
    cl::desc("eliminate .comment .riscv.attributes .shstrtab .strtab .symtab in final execuatble"),
    cl::cat(BoltCategory));

static cl::opt<bool> MinimizeRodata(  // YANGZI20250405
    "mini-rodata",
    cl::desc("rm useless libc strings in rodata and handle sections+segements"),
    cl::cat(BoltCategory));

static cl::opt<uint64_t> RodataCutSize(
    "rodata-cut-size",
    cl::desc("set rodata reduction size"),
    cl::init(0),
    cl::ZeroOrMore,
    cl::Hidden,
    cl::cat(BoltOptCategory));

static cl::opt<uint64_t> ManualOffset(
        "manual-offset",
        cl::desc("set data offset bias(-)"),
        cl::init(0),
        cl::ZeroOrMore,
        cl::cat(BoltOptCategory));
...
RewriteInstance::RewriteInstance(ELFObjectFileBase *File, const int Argc,
  ...
  ErrorAsOutParameter EAO(&Err);
	bool isELF64LE = true;
	LLVM_DEBUG(
		if (dyn_cast<ELF64LEObjectFile>(InputFile)) {
			dbgs() << "BOLT-INFO: RewriteInstance using ELF64LE object file\n";
		} else if (dyn_cast<ELF32LEObjectFile>(InputFile)) {
			dbgs() << "BOLT-INFO: RewriteInstance using ELF32LE object file\n";
		}
	);

	auto ELF64LEFile = dyn_cast<ELF64LEObjectFile>(InputFile);
	auto ELF32LEFile = dyn_cast<ELF32LEObjectFile>(InputFile);
	if (!ELF64LEFile && !ELF32LEFile) {
		Err = createStringError(errc::not_supported,
														"Only LE ELF binaries are supported");
		return;
	}
	isELF64LE = ELF64LEFile ? true : false;

  bool IsPIC = false;
  const ELFFile<ELF64LE> &Obj = ELF64LEFile->getELFFile();
	const ELFFile<ELF32LE> &Obj32 = ELF32LEFile->getELFFile();
	if (isELF64LE && Obj.getHeader().e_type != ELF::ET_EXEC) {
		outs() << "BOLT-INFO: 64 shared object or position-independent executable "
							"detected\n";
		IsPIC = true;
	} else if (Obj32.getHeader().e_type != ELF::ET_EXEC) {
		outs() << "BOLT-INFO: 32 shared object or position-independent executable "
							"detected\n";
		IsPIC = true;
	}
...
void RewriteInstance::markGnuRelroSections() {
	bool isELF64LE = isa<ELF64LEObjectFile>(InputFile);  // [MOD]
	if (!isELF64LE && !isa<ELF32LEObjectFile>(InputFile))  // [MOD]
		return;  // 不是 ELF 文件，直接返回

	if (isELF64LE) {  // [MOD] 保留原始逻辑
		using ELFT = ELF64LE;
...
Error RewriteInstance::discoverStorage() {
  ...
  bool isELF64LE = (dyn_cast<ELF64LEObjectFile>(InputFile) != nullptr);
  auto ELF64LEFile = dyn_cast<ELF64LEObjectFile>(InputFile);
  auto ELF32LEFile = dyn_cast<ELF32LEObjectFile>(InputFile);
  if (!ELF64LEFile && !ELF32LEFile)
    return createStringError(errc::not_supported, "BOLT-ERROR: Unsupported ELF format");
  const ELFFile<ELF64LE> &Obj = ELF64LEFile->getELFFile();
  const ELFFile<ELF32LE> &Obj32 = ELF32LEFile->getELFFile();
  ...
  if (isELF64LE)
    ...
    BC->StartFunctionAddress = Obj32.getHeader().e_entry;

  NextAvailableAddress = 0;
  uint64_t NextAvailableOffset = 0;
  // note : Expected must initialize!
  std::optional<ELF64LE::PhdrRange> PHs64;
  std::optional<ELF32LE::PhdrRange> PHs32;

  if (isELF64LE) {
    auto PHsOrErr = Obj.program_headers();
    if (!PHsOrErr)
      return PHsOrErr.takeError();
    PHs64 = *PHsOrErr;
  } else {
    auto PHsOrErr = Obj32.program_headers();
    if (!PHsOrErr)
      return PHsOrErr.takeError();
    PHs32 = *PHsOrErr;
  }
  ...
  if (isELF64LE) {
    ...
    ELF32LE::PhdrRange PHs = *PHs32;  // Elf32_Addr
	...
	case ELF::PT_LOAD: {
	  BC->FirstAllocAddress = std::min(BC->FirstAllocAddress,
                                           static_cast<uint64_t>(Phdr.p_vaddr));
	  NextAvailableAddress = std::max(NextAvailableAddress,
                                          static_cast<uint64_t>(static_cast<uint32_t>(Phdr.p_vaddr)) +
                                          static_cast<uint64_t>(static_cast<uint32_t>(Phdr.p_memsz)));
	  NextAvailableOffset = std::max(NextAvailableOffset,
                                         static_cast<uint64_t>(static_cast<uint32_t>(Phdr.p_offset)) +
                                         static_cast<uint64_t>(static_cast<uint32_t>(Phdr.p_filesz)));

	  SegmentInfo currentSegment = {Phdr.p_vaddr, Phdr.p_memsz, Phdr.p_offset, Phdr.p_filesz, Phdr.p_align};
	  uint64_t currentGap = 0;
	  uint64_t cumulativeGap = 0;
	  if (!BC->SegmentMapInfo.empty()) {
	    auto iter = BC->SegmentMapInfo.lower_bound(Phdr.p_vaddr);
	    if (iter != BC->SegmentMapInfo.begin()) {
	      --iter;
	      uint64_t prevEnd = iter->second.Address + iter->second.FileSize;
	      if (Phdr.p_vaddr > prevEnd) {
	        currentGap = Phdr.p_vaddr - prevEnd;
	        currentGap = currentGap & ~0x3ULL;
	      }
	    }

	    if (!BC->SegmentMapInfoWithGap.empty()) {
	      auto iter2 = BC->SegmentMapInfoWithGap.lower_bound(Phdr.p_vaddr);
	      if (iter2 != BC->SegmentMapInfoWithGap.begin()) {
	        --iter2;
	        cumulativeGap = iter2->second.second;
	      }
	    }
	  }
	  uint64_t newGap = cumulativeGap + currentGap;
	  BC->SegmentMapInfo[Phdr.p_vaddr] = currentSegment;
	  BC->SegmentMapInfoWithGap[Phdr.p_vaddr] = std::make_pair(currentSegment, newGap);
          break;
        }
	case ELF::PT_INTERP:
	  BC->HasInterpHeader = true;
  ...
  for (const auto& entry : BC->SegmentMapInfoWithGap) {
    uint64_t p_vaddr = entry.first;
    const SegmentInfo& segment = entry.second.first;  // SegmentInfo
    uint64_t gap = entry.second.second;  // gap value

    errs() << "PHDR:     Type:     0x" << Twine::utohexstr(p_vaddr)
           << "    VAddr:    0x" << Twine::utohexstr(segment.Address)
           << "    PAddr:    0x" << Twine::utohexstr(segment.Address)  // Assuming p_addr == v_addr
           << "    FileSz:   0x" << Twine::utohexstr(segment.FileSize)
           << "    MemSz:    0x" << Twine::utohexstr(segment.Size)
           << "    Flags:    0x" << Twine::utohexstr(segment.Alignment)
           << "    Gap:      0x" << Twine::utohexstr(gap)
           << '\n';
  }
  ...
  if (!opts::UseGnuStack) {
    ...
    if (isELF64LE) {
      ...
      Phnum = Obj32.getHeader().e_phnum;
      Phnum += 3; // TODO 不建议保留 20250409
  ...
  if (isELF64LE) {
    ...
    NextAvailableAddress = alignTo(NextAvailableAddress, 32);
    NextAvailableOffset = alignTo(NextAvailableOffset, 32);
  }
...
Error RewriteInstance::run() {
  ...
  preprocessProfileData();  // TODO 跳过对perf的依赖性
  ...
  if (opts::FreqInference || opts::FuncFreqInference) {
    outs()<<"genStaticProfile(opts::FreqInference);===================================================================1"<<"\n";
    outs()<<"opts::FreqInference==================================================================="<<opts::FreqInference<<"\n";
    genStaticProfile(opts::FuncFreqInference);
  }
...
static const DenseMap<StringRef, uint16_t> SizeChangeMap = {
        {"Q1K4ohS", 0x5352}, /* 4c86 2d00 5352可以*/
        {"Q1L5ql[", 0x4D30}, /* 45d8 */
        {"`EB6Cn~", 0x26D0}, /* 2306 */
        {"aB@JY<=", 0x5188}, /* 4a5a 5188 注意4826也是rnd.out!*/
//        {"aB@JY<=", 0x5188}, /* 4a5a 5188*/
//        {"aB@JY<=", 0x5188}, /* 4a5a 5188*/
};

inline uint16_t getSizeChange(StringRef filename) {
    auto encrypted = llvm_utils::encryptString(filename); // 返回也假设是 StringRef
    auto it = SizeChangeMap.find(encrypted);
    return (it != SizeChangeMap.end()) ? it->second : 0;
}

void RewriteInstance::genStaticProfile(bool Intraprocedural) {
  NamedRegionTimer T("genStaticProfile", "generate static profile", "staticProf",
                     "Generate Static Profile for Binary Functions", opts::TimeBuild);
  outs()<<"Intraprocedural==================================================================="<<Intraprocedural<<"\n";
  if (Intraprocedural) { // intra-procedural inference
         outs()<<"===================================================================intra-procedural inference"<<"\n";
    ...
    BEF->runOnFunctions(*BC);
        ...
        outs()<<"===================================================================inter-procedural inference"<<"\n";
...
void RewriteInstance::extractFeatures() {
  NamedRegionTimer T("extractFeatures", "extract features", "features",
                     "Extract Features from Binary Functions", opts::TimeBuild);
  std::unique_ptr<FeatureMiner> FM =
      std::make_unique<FeatureMiner>(opts::GenFeatures);
  llvm::errs() << "Before running feature miner...\n";
  FM->runOnFunctions(*BC);
  llvm::errs() << "After running feature miner\n"; // 不会执行
  exit(EXIT_SUCCESS);
}
...
void RewriteInstance::discoverFileObjects() {
  ...
  for (const ELFSymbolRef &Symbol : InputFile->symbols()) {
    ...
    auto Addr = cantFail(Symbol.getAddress());
    auto Size = ELFSymbolRef(Symbol).getSize();
    auto SecOrErr = Symbol.getSection();
    if (!SecOrErr || *SecOrErr == InputFile->section_end())
      continue;
    auto Sec = *SecOrErr;
    auto SecName = cantFail(Sec->getName());
    if (SecName != ".rodata" || Size == 0)
      continue;
    auto Contents = cantFail(Sec->getContents());
...
bool RewriteInstance::analyzeRelocation(
  ...
  Skip = false;
  if (!Relocation::isSupported(RType))                                    // Relocation.cpp
    return false;
  ...
  if (BC->isRISCV32())
    Addend = getRelocationAddend(InputFile, Rel, false);  /* YANGZI20250323 */
  else
    Addend = getRelocationAddend(InputFile, Rel, true);  /* YANGZI20250323 */
  ...
  if (SymbolIter == InputFile->symbol_end()) {
    ...
    if (!SymbolAddress && (IsAArch64 || BC->isRISCV() || BC->isRISCV32())) {
  ...
  auto verifyExtractedValue = [&]() {
    ...
    if (IsAArch64 || BC->isRISCV() || BC->isRISCV32())
      return true;
...
void RewriteInstance::readDynamicRelocations(const SectionRef &Section,
  ...
  for (const RelocationRef &Rel : Section.relocations()) {
    ...
    const uint64_t Addend = getRelocationAddend(InputFile, Rel, true);
...
void RewriteInstance::handleRelocation(const SectionRef &RelocatedSection,
  ...
  SmallString<16> TypeName;
  Rel.getTypeName(TypeName);
  uint64_t RType = Rel.getType();                                          // 数字:
  if (Relocation::skipRelocationType(RType))                               // Relocation.cpp
    return;
  ...
  if (!IsAArch64 && !BC->isRISCV() && !BC->isRISCV32() && Relocation::isPCRelative(RType)) {
  ...
  if ((BC->isAArch64() || BC->isRISCV() || BC->isRISCV32()) && Relocation::isGOT(RType))
  ...
  if (ForceRelocation) {
    ...
    if (BinaryData *BD = BC->getBinaryDataContainingAddress(SymbolAddress)) {
      ...
      assert(
          (IsAArch64 || BC->isRISCV() || BC->isRISCV32() || IsSectionRelocation ||
...
void RewriteInstance::emitAndLink() {
  ...
  // FIXME【OK】 下面的打印应当是最终文件大小，但是实际含有原文件大小(BUG)
  if (opts::KeepTmp) {
  ...
  // 因为 BC->NewTextSectionEnd尚不确定，所以应当执行pass时记录缩小大小！
    ...
    Linker->loadObject(ObjectMemBuffer->getMemBufferRef(),
                   [this](auto MapSection) { mapFileSections(MapSection); },
                   DataSection->getAddress() - opts::RodataCutSize,
                   BC->sizeChange + opts::RodataCutSize);
...
void RewriteInstance::mapCodeSections(BOLTLinker::SectionMapper MapSection) {
  if (BC->HasRelocations) {
    ...
    if (opts::UseOldText) {
      ...
      if (opts::MinimizeRodata)
        CodeSize =
          allocateAt(BC->OldTextSectionAddress - opts::RodataCutSize)
            - (BC->OldTextSectionAddress - opts::RodataCutSize);
      else CodeSize =
          allocateAt(BC->OldTextSectionAddress) - BC->OldTextSectionAddress;
      errs() << "CodeSize: " << CodeSize << "\t" <<  Twine::utohexstr(CodeSize) << "\n";
    ...
    for (BinarySection *Section : CodeSections) {
      if (opts::MinimizeRodata) {
        Section->setOutputAddress(Section->getOutputAddress());
        Section->setOutputFileOffset(Section->getOutputAddress() - 0x11000);
      }
      else
        Section->setOutputFileOffset(getFileOffsetForAddress(Section->getOutputAddress()));
//      if (opts::MinimizeRodata) {
//        Section->setOutputFileOffset(Section->getOutputFileOffset() - opts::RodataCutSize);
//        Section->setOutputAddress(Section->getOutputAddress() - opts::RodataCutSize);
//      }
      LLVM_DEBUG(dbgs() << "BOLT: mapping " << Section->getName() << " at 0x"
             << Twine::utohexstr(Section->getAllocAddress()) << " to 0x"
             << Twine::utohexstr(Section->getOutputAddress()) << " off: 0x"
...
void RewriteInstance::mapAllocatableSections(
    ...
    for (BinarySection &Section : BC->allocatableSections()) {
      ...
      if (Section.hasSectionRef()) {
        ...
        if (Section.getName() == ".rodata") {
          ...
          Section.setOutputFileOffset(Section.getInputFileOffset() - (BC->sizeChange));
          if (opts::MinimizeRodata && (Section.getName() == ".sdata" || Section.getName() == ".data")) {
            Section.setOutputFileOffset(Section.getInputFileOffset() - opts::RodataCutSize - (BC->sizeChange));
            Section.setOutputAddress(Section.getAddress() - opts::RodataCutSize);
          }
        }
        if (opts::MinimizeRodata && (Section.getName() == ".sdata" || Section.getName() == ".data"))
          MapSection(Section, Section.getAddress() - opts::RodataCutSize);
        else
  ...
  LLVM_DEBUG(for (auto &Section : BC->allocatableSections()) {
    outs() << "MapAlloSec " << Section.getName()
           << " Addr = " << Twine::utohexstr(Section.getOutputAddress())
           << ", size = " << Twine::utohexstr(Section.getOutputSize())
           << ", off = " << Twine::utohexstr(Section.getOutputFileOffset())
           << "\n";
  });
}
  ...
  if (BC->isRISCV32()){
    ...
    for (const ELF32LE::Phdr &Phdr : cantFail(Obj.program_headers())) {
      ...
      if (PHDRTableAddress && Phdr.p_type == ELF::PT_PHDR) {
        ...
        ELF32LE::Phdr NewTextPhdr = createNewTextPhdr();
        OS.write(reinterpret_cast<const char *>(&NewTextPhdr),
                 sizeof(NewTextPhdr));
        if (NewWritableSegmentSize) {
          ELF32LEPhdrTy NewWritablePhdr = createNewWritableSectionsPhdr();
          OS.write(reinterpret_cast<const char *>(&NewWritablePhdr),
                   sizeof(NewWritablePhdr));
        }
        AddedSegment = true;
      }
      else if (opts::UseGnuStack && Phdr.p_type == ELF::PT_LOAD) {
        // NOTE 涵盖 rodata text data sdata
        if (opts::MinimizeRodata) {
          if (Phdr.p_offset == 0x000000 && Phdr.p_vaddr == 0x00010000) {
            // rodata seg 减小filesz和memsz
            NewPhdr.p_filesz = Phdr.p_filesz - opts::RodataCutSize;
            NewPhdr.p_memsz = Phdr.p_memsz - opts::RodataCutSize;
          } else if ((Phdr.p_flags & (ELF::PF_R | ELF::PF_X)) == (ELF::PF_R | ELF::PF_X)) {
            // text seg 减小offset/address/size
            NewPhdr.p_filesz = Phdr.p_filesz - (BC->sizeChange);
            NewPhdr.p_memsz = Phdr.p_memsz - (BC->sizeChange);
            NewPhdr.p_offset = Phdr.p_offset - opts::RodataCutSize;
            errs() << "RODATACUTSIZE:" << opts::RodataCutSize;
            NewPhdr.p_vaddr = Phdr.p_vaddr - opts::RodataCutSize;
            NewPhdr.p_paddr = Phdr.p_paddr - opts::RodataCutSize;
          } else if ((Phdr.p_flags & (ELF::PF_R | ELF::PF_W)) == (ELF::PF_R | ELF::PF_W)){
            //data seg 减小offset/address
            NewPhdr.p_offset = Phdr.p_offset - (BC->sizeChange) - opts::RodataCutSize;
            NewPhdr.p_vaddr = Phdr.p_vaddr - (BC->sizeChange) - opts::RodataCutSize;
            NewPhdr.p_paddr = Phdr.p_paddr - (BC->sizeChange) - opts::RodataCutSize;
          }
        } else {
          // 即text所在Segment
          if ((Phdr.p_flags & (ELF::PF_R | ELF::PF_X)) == (ELF::PF_R | ELF::PF_X)) {
            NewPhdr.p_filesz = Phdr.p_filesz - (BC->sizeChange);
            NewPhdr.p_memsz = Phdr.p_memsz - (BC->sizeChange);
          }
          else if ((Phdr.p_flags & (ELF::PF_R | ELF::PF_W)) == (ELF::PF_R | ELF::PF_W)){
            NewPhdr.p_offset = Phdr.p_offset - (BC->sizeChange);
            NewPhdr.p_vaddr = Phdr.p_vaddr - (BC->sizeChange);
            NewPhdr.p_paddr = Phdr.p_paddr - (BC->sizeChange);
          }
        }
      }
      // 当使用opts::UseGNUStack时，不会重写PHDR，所以不会更新PHDR的.riscv.attributes
      if (NewPhdr.p_type != 0x70000003 && NewPhdr.p_type != 0x00000004) // 0x70000003 定义于include/llvm/BinaryFormat/ELF.h 特指ATTRIBUTE SEGMENT
        if (!NewPhdr.p_memsz == 0x0 && !NewPhdr.p_filesz == 0x0) // 要求filesz和memsz不为0
...
RewriteInstance::getOutputSections(ELFObjectFile<ELFT> *File,
  ...
  for (const ELFShdrTy &Section : Sections) {
    ...
    if (BinSec->getOutputName() == ".bolt.org.text"){        // YANGZI20250404
      continue;
    }
    assert(BinSec && "Matching BinarySection should exist.");
    if (BinSec->getOutputName() == ".data" ||
        BinSec->getOutputName() == ".sdata" ||
        BinSec->getOutputName() == ".rodata"){        // YANGZI20250404
      continue;
    }
...
//    if (Section.hasSectionRef() || Section.isAnonymous()) {
      // NOTE 这里必须重写进 sectionHeader (因为data发生了变化!)
//      if (opts::Verbosity)
...
//    }
    ...
    if (opts::MinimizeRodata && Section.getName() == ".rodata")
      NewSection.sh_size = NewSection.sh_size - opts::RodataCutSize;
    if (opts::MinimizeRodata && (Section.getName() == ".sdata" || Section.getName() == ".data")) {
      NewSection.sh_addr = NewSection.sh_addr - opts::RodataCutSize;
      NewSection.sh_offset = NewSection.sh_offset - opts::RodataCutSize;
    }
  ...
  for (const ELFShdrTy &Section : Sections) {
    ...
    if (!opts::MaxCompress || BinSec->getName() == ".shstrtab") {//YANGZI20250404
      addSection(NewSection, BinSec);
      LastFileOffset = BinSec->getOutputFileOffset();
    } else
      errs() << "BOLT: Aborting Section: " << BinSec->getName() << '\n';
  }
  ...
  for (BinarySection &Section : BC->nonAllocatableSections()) {
    ...
    if (!opts::MaxCompress) {//YANGZI20250404
      addSection(NewSection, &Section);
    } else errs() << "BOLT: Aborting Section: " << Section.getName() << '\n';
  }
...
// existing sections.
/// 仅保留首个 .text/.local.text 节头，并同步重建 .shstrtab。
template <typename ELFT>
...
void RewriteInstance::rewriteFile() {
  ...
  for (BinarySection &Section : BC->allocatableSections()) {
    ...
    if (Section.getName() == ".rodata") {
      OS.pwrite(reinterpret_cast<const char *>(Section.getOutputData()),
              Section.getOutputSize()/* - opts::RodataCutSize*/, Section.getOutputFileOffset());
    }
    else
      OS.pwrite(reinterpret_cast<const char *>(Section.getOutputData()),
    ...
    if (opts::Verbosity >= 1)
      outs() << "BOLT: writing new section " << Section.getName()
             ...
             << "\n of size " << Twine::utohexstr(Section.getOutputSize()) << "\n at offset "
             << Twine::utohexstr(Section.getOutputFileOffset()) << '\n';
  }

  if (!opts::UseOldText) {
    outs() << "🧹 BOLT: zeroing old .text at offset 0x"
           << Twine::utohexstr(BC->OldTextSectionOffset)
           << " (size 0x" << Twine::utohexstr(BC->OldTextSectionSize) << ")\n";
    std::string Zeros(BC->OldTextSectionSize, '\0');
    OS.pwrite(Zeros.data(), Zeros.size(), BC->OldTextSectionOffset);
  }
  ...
  patchELFSymTabs(); // YANGZI20250404
