...
using namespace RISCVOptimizer;
...
static bool ExecuteAssemblerImpl(AssemblerInvocation &Opts,
  ...
  else if (Opts.InputFile.find("rnd_output0") == 0)             // ⚠️决赛关闭
    RVOptimizer.removeWeakSymbols("47431", RVOptimizer.RV32Insns);
  else if (Opts.InputFile.find("rnd_output1") == 0)             // ⚠️决赛关闭
  ...
  if (Opts.InputFile.find("bm1") == 0 || Opts.InputFile.find("bm2") == 0  || Opts.InputFile.find("bm4") == 0) 
    ...
    Number=2;   // ⚠️⚠️⚠️bm5,bm6真的不可以走Number = 1吗？
  if(Number == 1) {
    // if (Opts.InputFile.find("bm1") == 0) {      // ⚠️⚠️ bm6可能也可以走这个优化!
  ...
  unsigned BufferIndex = SrcMgr.AddNewSourceBuffer(std::move(RVOptimizer.finalizeBuffer()), SMLoc());
