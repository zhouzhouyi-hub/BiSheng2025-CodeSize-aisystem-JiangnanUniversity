...
private:
  Error applyFixup(LinkGraph &G, Block &B, const Edge &E) const {
    ...
    case R_RISCV_32: {
      ...
      if ((E.getTarget().getAddress() + E.getAddend()).getValue() >= DataSectionBegin) {
        errs() << "RV32目标地址:" << E.getTarget().getAddress() << "\n";
        Value -= FixupOffset;
      }
    ...
    case R_RISCV_HI20: {
      int64_t Value = (E.getTarget().getAddress() + E.getAddend()).getValue();
      if ((E.getTarget().getAddress() + E.getAddend()).getValue() >= DataSectionBegin) {
        errs() << "HI20目标地址:" << E.getTarget().getAddress() << "\n";
        Value -= FixupOffset;
      }
      int64_t Hi = Value + 0x800;
      if (LLVM_UNLIKELY(!isInRangeForImm(Hi, 32)))
    ...
    case R_RISCV_LO12_I: {
      ...
      int64_t Value = (E.getTarget().getAddress() + E.getAddend()).getValue();
      if ((E.getTarget().getAddress() + E.getAddend()).getValue() >= DataSectionBegin) {
        errs() << "LO12目标地址:" << E.getTarget().getAddress() << "\n";
        Value -= FixupOffset;
      }
      int32_t Lo = Value & 0xFFF;
      uint32_t RawInstr = *(little32_t *)FixupPtr;
...
void link_ELF_riscv(std::unique_ptr<LinkGraph> G,
  ...
  ELFJITLinker_riscv::link(std::move(Ctx), std::move(G), std::move(Config), dataSectionBegin, fixupOffset);
}
